...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        self.models = ['det1', 'det2', 'det3','det4']
        self.models = [ os.path.join(model_folder, f) for f in self.models]
        # models = ['det1', 'det2', 'det3','det4']
        # models = [ os.path.join(model_folder, f) for f in models]
        
        self.PNets = []
        for i in range(num_worker):
            workner_net = mx.model.FeedForward.load(self.models[0], 1, ctx=ctx)
            self.PNets.append(workner_net)
        self.Pool = Pool(num_worker)

        # self.RNet = mx.model.FeedForward.load(models[1], 1, ctx=ctx)
        self.ONet = mx.model.FeedForward.load(self.models[2], 1, ctx=ctx)
        self.LNet = mx.model.FeedForward.load(self.models[3], 1, ctx=ctx)
    ...
    def detect_face(self, img):
        ...
        RNet = mx.model.FeedForward.load(self.models[1], 1, ctx=mx.cpu())
        output = RNet.predict(input_buf)# modify by bobi 2018.09.16
